...
- **M4(AECLAW 真实链路验证)离线部分已实证** —— 把 M1 采集 7 类拷进真实项目 `ae-negotiation-ai` 的 test 包(`com.ae.negotiation.threadmap`,零碰 production),用窄 `@SpringBootTest`(命令行禁 nacos、`@MockBean` 外部依赖)在真实红线领域链路 `RedlineDomainService#check → RuleConfigLoader / RuleEngineService` 上做 Bean 级 AOP 采集,产出 22 节点真实 `trace.json`;离线 `ThreadmapCli`(`runCli` Gradle 任务)+ AECLAW 源码根 + 包白名单 `com.ae.negotiation` 产出 `annotated-tree.json`;`MethodSourceExtractor` 实证能从 AECLAW 真实源码抽到 `check` 方法源码 + 被调名。采集→折叠→源码抽取→标注整条链路在真实业务代码上跑通(标注暂用离线 `FakeAnnotator`)。**真实 `qwen3.6-flash` 标注质量核对待用户带 `DASHSCOPE_API_KEY` 执行**(按安全规则不由助手处理明文 key)。
- **下一步:** M4 收尾(用户带 key 跑 `runCli` 切真实 qwen,核对标注质量、迭代 prompt / 折叠规则);随后 M3(IntelliJ 插件,读 `annotated-tree.json` 渲染右侧栏树)。
