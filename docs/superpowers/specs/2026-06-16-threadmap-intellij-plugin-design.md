...
- **M2b-2(源码抽取 + 流水接线 + CLI)已实现** —— `MethodSourceExtractor`(JavaParser 按 `signature` 定位方法、抽方法源码 `toString()` + 直接被调方法名)→ `SourceBackedRequestBuilder` 经 `AnnotationRequestBuilder` 接缝注入 `AnnotationPipeline`(3-arg 构造器;2-arg 旧构造器保留、默认仅签名)→ `ThreadmapCli`(可测 `run(...)` + `main(...)` 组装真实 Qwen 链 / 无 key 降级 `FakeAnnotator`,写 `.threadmap/annotated-tree.json`)。顺带加固 `TraceJsonParser`(三顶层字段 `requireField` 校验)+ `CachingAnnotator`(键加 `src:`/`sig:` 前缀)。全程离线可测,**53 测试全绿**。计划见 [plans/2026-06-17-threadmap-core-m2b2-source-extraction-cli.md](../plans/2026-06-17-threadmap-core-m2b2-source-extraction-cli.md)。至此 **M2(core 引擎)完整**:采集(M1)→ 折叠 + 真实标注 + 缓存(M2a/M2b)→ `annotated-tree.json`。
- **M4(AECLAW 真实链路端到端验证)已完整跑通** —— 把 M1 采集 7 类拷进真实项目 `ae-negotiation-ai` 的 test 包(`com.ae.negotiation.threadmap`,零碰 production),用窄 `@SpringBootTest`(命令行禁 nacos、`@MockBean` 外部依赖)在真实红线领域链路 `RedlineDomainService#check → RuleConfigLoader / RuleEngineService` 上做 Bean 级 AOP 采集,产出 22 节点真实 `trace.json`;`ThreadmapCli`(`runCli` Gradle 任务)+ AECLAW 源码根 + 包白名单 `com.ae.negotiation` 产出 `annotated-tree.json`。**真实 `qwen-flash` 标注质量良好**:`check` 节点 summary"根据规则校验红线并生成违规列表"、inputs/outputs 准确、`dig_reason` 点出"核心业务逻辑关键执行体"+ 子节点指出"潜在空指针""强制类型转换容错";`evidence.calls` 含 `buildVarMap`/`resolveValue` 等**同 Bean 自调用**(AOP 拦不到、由 JavaParser 从源码补足,印证 M2b-2 source-backed 标注的价值)。**修了一个真实 bug**:`QwenChatModels.DEFAULT_MODEL` 原为 OpenAI compatible-mode id `qwen3.6-flash`,对 DashScope 原生 Generation API 无效("url error"),改为原生 `qwen-flash`。
- **M3a(IntelliJ 插件骨架 + 列式调用树)已实现并 runIde 实证** —— `threadmap-intellij`(Kotlin,IntelliJ Platform Gradle Plugin 2.x,基线 IDEA Community 2024.2)依赖 core(exclude 引擎重依赖、只带 jackson + 模型);core 加 `AnnotatedTreeJsonReader`(对称 writer);可测逻辑 `CallTreeNodeBuilder` + `NodePresentation`(纯 JVM 测),薄壳 `TreeTableView` 四列(签名/摘要/状态/副作用)+ `StatusStripeRenderer`(左侧状态色条)+ `ThreadmapPanel`(ActionToolbar 加载/重载 + 启动 loadDefault)。**runIde 在真实 IDE 里实证**:加载 AECLAW 真实 `annotated-tree.json`,渲染出可折叠调用树 + qwen 中文摘要 + 灰色状态条。core 55 + 插件 5 测试绿。计划见 [plans/2026-06-18-threadmap-m3a-plugin-skeleton-tree.md](../plans/2026-06-18-threadmap-m3a-plugin-skeleton-tree.md);实现设计见 [specs/2026-06-18-threadmap-m3-intellij-plugin-impl.md](2026-06-18-threadmap-m3-intellij-plugin-impl.md)。
- **下一步:** M3b(选中节点详情面板 + PSI 按 signature 跳源码) → M3c(进度持久化 + 待查清单 + 导出 todo + 过滤)。可选打磨:列默认宽度自适应、拆轻量 `threadmap-model` 模块减插件臃肿。
...
- **M2b-2 已知/接受的抽取与 CLI 限制**(M4 前评估):`MethodSourceExtractor` 按 方法名 + 参数个数 匹配,重载取首个;嵌套类已按最内层 `$` 简单名 scope 定位(终审修复,见 `1efd95f`),但匿名 / 局部类不支持。被调名是**裸方法名**,填入 `AnnotationRequest.calleeSignatures` 仅作 prompt 提示(非完整签名,M4 前考虑升级 symbol-resolved)。方法源码用 `toString()`(丢原始格式 / 注释,`LexicalPreservingPrinter` 推迟)。`ThreadmapCli.main` 暂无顶层 try/catch(异常以堆栈传播、退出码靠 JVM 默认)、仅接受单个 include 包前缀。FQCN→路径推导在 `MethodSourceExtractor` 与 `FakeAnnotator` 有重复(可抽 `Signatures` helper)。`QwenAnnotator` 降级日志仍待引入日志框架。
